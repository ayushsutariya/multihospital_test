...
import { Add_Auth_Data, Get_Auth_Data } from "../Action/Auth.action";
function Auth(props) {
  const [uselogin, setlogin] = useState("Login");
  const [usepassword, setpassword] = useState(false); 
  const dispatch = useDispatch();
  const Auth_Display = useSelector(state => state.Authreducer)
  console.log(Auth_Display.Auth_data);
  console.log(Auth_Display.Authreducer);

  useEffect(() => {
    dispatch(Get_Auth_Data())
  }, [])
  ...
  let schema, initVal;
  if (uselogin === "Login" && usepassword === false) {
    schema = yup.object().shape(login_set);
    initVal = {
      Email: "",
      Password: "",
    };
  } else if (uselogin === "Signup" && usepassword === false) {
              ...
              <Formik value={formik}>
                <Form className="mb-3" onSubmit={formik.handleSubmit}>
                  <div className="form-group first mb-3">
                    {uselogin === "Login" || usepassword === true ? null : (
                      <div>
